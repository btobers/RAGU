...
def read_h5(fpath, navcrs, body):
    ...
    if "srf0" in f["ext"].keys():
        rdata.set_gndHgt(f["ext"]["srf0"][:])                                  # surface elevation from lidar, averaged over radar first fresnel zone per trace (see code within /zippy/MARS/code/xped/hfProc/ext)
        # read in existing surface pick
        if "twtt_surf" in f["drv"]["pick"].keys():
            rdata.pick.existing_twttSurf = f["drv"]["pick"]["twtt_surf"][:]
        else:
            # calculate twtt_surf
            rdata.pick.existing_twttSurf = utils.depth2twtt(rdata.navdf["elev"] - rdata.gndHgt, eps_r=1)
    # create empty arrays to hold surface elevation and twtt otherwise
    else:
        rdata.set_gndHgt(np.repeat(np.nan, rdata.tnum))
...
def read_mat(fpath, navcrs, body):
    ...
    rdata.set_gndHgt(rdata.navdf["hgt"].to_numpy() - dist)
